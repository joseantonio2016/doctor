...
class WebsiteController extends Controller
{
    public function index()
    {
        ...
        $reviews = Review::get();
        $blogs = Blog::get();
        return view('website.home-k', compact('banners', 'doctors', 'treatments', 'setting', 'reviews', 'blogs'));
    }

    public function seParte(Request $request)
    {
        return view('website.separte');
    }
    ...
    public function doctor(Request $request)
    {
        ...
        if ($request->ajax()) {
            $view = view('website.display_doctors1', compact('doctors', 'currency', 'categories'))->render();
            return response()->json(['html' => $view, 'count' => count($doctors), 'meta' => $doctors, 'success' => true]);
        }
        return view('website.find_doctor-k', compact('doctors', 'currency', 'categories'));
    }
